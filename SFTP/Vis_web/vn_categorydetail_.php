...
<FORM id=Form name=Form action=../S-Portal.aspx?tabid=27 method=post encType=multipart/form-data>
	...
	<tr>
<?
$SQL3 = "SELECT * FROM Category where PCID=$_GET[id]";
...
echo '<tr>';
echo '<td>';
echo '<div align="center">'.$row3["Photo"].'</div>'; 
echo '</td>';
echo '</tr>';
echo '<tr>';
echo '<td>';
echo '<div align="center">';
if ($row3["Queue"]=='1'){echo '<a href=vn_product.php?id='.$row3["CategoryID"].'>'.$row3["EngName"].'</a>';}
else {echo '<a href=vn_category.php?id='.$row3["CategoryID"].'>'.$row3["EngName"].'</a>';}
echo '</div>';
echo '</td>';
echo '</tr>';
echo '</table>';
