...
function print_menu($mid, $line = 0, $perline = 0) {
	...
	$start = ($line) * $perline;
	$sql = "SELECT * FROM Menu WHERE PCID=$mid AND ActiveStatus='T' AND ShowInIndex='F' LIMIT $start, $perline";
	//echo $sql;
	...
	while ($row = mysql_fetch_array($result)) {
		...
		if ($row["OutletID"] > 0) {
			...
			$javascript_menu_item .= 'fx("text='.$row['EngName'].' ;show='.$row['CategoryID'].';url=vn_vn_menudetail.php?id='.$row['CategoryID'].';'.(($mid==0)?('fontweight=bold;'):('')).'target=iframe;width=175;height='.$hg.'");';
			...
			{
				$javascript_menu_item .= 'fx("text='.$row['EngName'].' ;show='.$row['CategoryID'].';url=vn_menudetail.php?id='.$row['CategoryID'].';'.(($mid==0)?('fontweight=bold;'):('')).'target=iframe;width=175;height='.$hg.'");';
          ...
          <TD colSpan=3>
            <TABLE id=table3 cellSpacing=0 cellPadding=0 width="100%" border=0>
              <TR>
                <TD bgColor=#dcdcdc>
                 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                 <a href="vn_search.php" target="iframe" style="font-weight:bold;">Tìm kiếm nâng cao</a>
                </TD>
                </TR>
              <TR>
                <TD bgColor=#dcdcdc>
