...
if ($recordCount>0)
{
	...
	while($row1 = mysql_fetch_array($rs1))
	{
		...
		{
		$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;url=contact.php?id='.$row1['CategoryID'].';target=iframe;width=145");';
		}
		else
		{
			$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;show='.$row1['CategoryID'].';url=contact.php?id='.$row1['CategoryID'].';target=iframe;width=145");';
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;url=contact.php?id='.$row2['CategoryID'].';target=iframe;width=145");';
				}
				else
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact.php?id='.$row2['CategoryID'].';target=iframe;width=145");';
						...
						while($row3 = mysql_fetch_array($rs3))
							{
								if($row3['Queue']=='0' && $row3['Queue']!='1')
									{
										$javascript_menu_item3.='fx("text='.$row3['EngName'].' ;url=contact.php?id='.$row3['CategoryID'].';target=iframe;width=145");';
									}
								else
									{
									$javascript_menu_item3.='fx("text='.$row3['EngName'].' ;show='.$row3['CategoryID'].';url=contact.php?id='.$row3['CategoryID'].';target=iframe;width=145");';
				// start level4
			...
			while($row4 = mysql_fetch_array($rs4))
			  {
			//if($row3['Queue']=='0')
				 { $javascript_menu_item4.='fx("text='.$row4['EngName'].' ;url=contact.php?id='.$row4['CategoryID'].';target=iframe;width=145");';}
			
				//else
			 // {
			//	$javascript_menu_item4.='fx("text='.$row4['EngName'].' >;show='.$row4['CategoryID'].';target=iframe;width=145");';
			 //  }
			}
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >4)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 4, 2";
	$rs0 = mysql_query($SQL);
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >8)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 7, 5";
	$rs0 = mysql_query($SQL);
		...
		else
		{
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				else
				{
					...
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact.php?id='.$row2['CategoryID'].';target=iframe;width=145;height=40;");';
					} else {				
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact.php?id='.$row2['CategoryID'].';target=iframe;width=145");';
					}
...
while($row0 = mysql_fetch_array($rs0))
{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
	 { $javascript_menu_item0.='fx("text='.$row0['EngName'].' ;url=vn_category.php?id='.$row0['CategoryID'].';target=iframe");';}
	else
	{
...
while($row1 = mysql_fetch_array($rs1))
{
	if($row1['Queue']=='0' && $row1['Queue']!='1')
	{
		$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;url=vn_category.php?id='.$row1['CategoryID'].';target=iframe;width=150");';
	}
	else
	{
		...
		while($row2 = mysql_fetch_array($rs2))
		{
			if($row2['Queue']=='0' && $row2['Queue']!='1')
			{
				$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;url=vn_category.php?id='.$row2['CategoryID'].';target=iframe;width=150");';
			}
			else
			{
