...
$SQL_c = "SELECT * FROM Menu WHERE PCID=0 and ShowInIndex='F' ORDER BY Position ASC";
...
if ($recordCount>0)
{
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 0, 4";
	$rs0 = mysql_query($SQL);
	
	$javascript_menu_item0 = '';
	while($row0 = mysql_fetch_array($rs0))
	{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
	 	{ $javascript_menu_item0.='fx("text='.$row0['EngName'].';url=contact_.php?id='.$row0['CategoryID'].'&menu0='.$row0['EngName'].';fontweight=bold;target=iframe");';}
		else
		{
		$javascript_menu_item0.='fx("text='.$row0['EngName'].';show='.$row0['CategoryID'].';url=vn_menudetail.php?id='.$row0['CategoryID'].';fontweight=bold;target=iframe");';
	...
	//level 1
	$SQL1 = "SELECT * FROM Menu where PCID=0 and ShowInIndex='F' and ActiveStatus='T'";
	$rs1 = mysql_query($SQL1);
	while($row1 = mysql_fetch_array($rs1))
	{
		if($row1['Queue']=='0' && $row1['Queue']!='1')
		{
		$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;url=contact_.php?id='.$row1['CategoryID'].';target=iframe;width=185");';
		}
		else
		{
			$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;show='.$row1['CategoryID'].';url=vn_menudetail.php?id='.$row1['CategoryID'].';target=iframe;width=185");';
		
			//level2
			 //sql
			$SQL2 = "SELECT * FROM Menu where PCID='$row1[CategoryID]' and ShowInIndex='F' and ActiveStatus='T'";
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;url=contact_.php?id='.$row2['CategoryID'].'&menu1='.$row2['EngName'].';target=iframe;width=185");';
				}
				else
				{
				if (strlen($row2['EngName']) > 20) {
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=vn_menudetail.php?id='.$row2['CategoryID'].';target=iframe;width=185;height=40;");';
				}
				else
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=vn_menudetail.php?id='.$row2['CategoryID'].';target=iframe;width=185");';
				}
				//level3
				//sql
				
				  	$SQL3 = "SELECT * FROM Menu where PCID='$row2[CategoryID]' and ShowInIndex='F' and ActiveStatus='T'";
						...
						while($row3 = mysql_fetch_array($rs3))
							{
								if($row3['Queue']=='0' && $row3['Queue']!='1')
									{
										$javascript_menu_item3.='fx("text='.$row3['EngName'].' ;url=contact_.php?id='.$row3['CategoryID'].';target=iframe;width=185");';
									}
								else
									{
									$javascript_menu_item3.='fx("text='.$row3['EngName'].' ;show='.$row3['CategoryID'].';url=vn_menudetail.php?id='.$row3['CategoryID'].';target=iframe;width=185");';
				// start level4
				
			$SQL4 = "SELECT * FROM Menu where PCID='$row3[CategoryID]' and ShowInIndex='F' and ActiveStatus='T'";
      	  $rs4 = mysql_query($SQL4);
			$javascript_menu_item4 = '';
			while($row4 = mysql_fetch_array($rs4))
			  {
			//if($row3['Queue']=='0')
				 { $javascript_menu_item4.='fx("text='.$row4['EngName'].' ;url=contact_.php?id='.$row4['CategoryID'].';target=iframe;width=185");';}
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >4)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 4, 1";
	$rs0 = mysql_query($SQL);
	
	$javascript_menu_item0 = '';
	while($row0 = mysql_fetch_array($rs0))
	{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
	 	{ $javascript_menu_item0.='fx("text='.$row0['EngName'].' ;url=contact_.php?id='.$row0['CategoryID'].';fontweight=bold;target=iframe");';}
		else
		{
		$javascript_menu_item0.='fx("text='.$row0['EngName'].' ;show='.$row0['CategoryID'].';url=vn_menudetail.php?id='.$row0['CategoryID'].';fontweight=bold;target=iframe");';
		}
	...
	while($row1 = mysql_fetch_array($rs1))
	{
		...
		else
		{
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				if($row2['Queue']=='0' && $row2['Queue']!='1')
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;url=contact_.php?id='.$row2['CategoryID'].';target=iframe;width=185");';
				}
				else
				{
				if (strlen($row2['EngName']) > 20) {
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >8)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 5, 1";
	$rs0 = mysql_query($SQL);
	...
	while($row1 = mysql_fetch_array($rs1))
	{
		...
		else
		{
			...
			$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;show='.$row1['CategoryID'].';url=contact_.php?id='.$row1['CategoryID'].';target=iframe;width=185;height=40;");';
			} else {	
			$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;show='.$row1['CategoryID'].';url=vn_menudetail.php?id='.$row1['CategoryID'].';target=iframe;width=185");';
			}
			//level2
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				{
				if (strlen($row2['EngName']) > 20) {
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact_.php?id='.$row2['CategoryID'].';target=iframe;width=185;height=40;");';
				} else {
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=vn_menudetail.php?id='.$row2['CategoryID'].';target=iframe;width=185;height=20;");';
				}
				//level3
				//sql
...
//echo $javascript_menu1;
 ...
 // Main Menu 3
 				...
 				if ($recordCount >8)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='F' ORDER BY Position LIMIT 6, 5";
	$rs0 = mysql_query($SQL);
...
height=20;
orientation="horizontal";
fx("text=Vine Cellar ;url=vn_categorydetail.php?id=1;fontweight=bold;show=Product;target=iframe");
...
$javascript_menu0.= '
					...
					';
echo $javascript_menu0;					
$sql0 = "SELECT * FROM Category where PCID=0 and ShowInIndex='F' and ActiveStatus='T'";
$rs0=mysql_query($sql0);
$cate='101010';
$javascript_menu_item0 = '';
while($row0 = mysql_fetch_array($rs0))
{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
	 { $javascript_menu_item0.='fx("text='.$row0['EngName'].' ;url=category_.php?id='.$row0['CategoryID'].';target=iframe");';}
	else
	{
		$javascript_menu_item0.='fx("text='.$row0['EngName'].';url=vn_categorydetail.php?id='.$row0['CategoryID'].';show='.$row0['CategoryID'].'101010;target=iframe");';
	}
}					
...
$javascript_menu_item1 = '';
//level 1
$SQL1 = "SELECT * FROM Category where PCID=0 and ShowInIndex='F' and ActiveStatus='T'";
$rs1 = mysql_query($SQL1);
while($row1 = mysql_fetch_array($rs1))
{
	if($row1['Queue']=='0' && $row1['Queue']!='1')
	{
		$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;url=category_.php?id='.$row1['CategoryID'].';target=iframe;width=150");';
	}
	else
	{
		...
		//level2
		 //sql
		$SQL2 = "SELECT * FROM Category where PCID='$row1[CategoryID]' and ShowInIndex='F' and ActiveStatus='T'";
        $rs2 = mysql_query($SQL2);
		$javascript_menu_item2 = '';
		while($row2 = mysql_fetch_array($rs2))
		{
			if($row2['Queue']=='0' && $row2['Queue']!='1')
			{
				$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;url=category_.php?id='.$row2['CategoryID'].';target=iframe;width=150");';
			}
			else
			{
				$javascript_menu_item2.='fx("text='.$row2['EngName'].';url=vn_product.php?id='.$row2['CategoryID'].';show='.$row2['CategoryID'].'101010;target=iframe;width=150");';
				...
				//$SQL3 = "SELECT * FROM ProductCategory where CategoryID='$row2[CategoryID]'";
       			$SQL3 = "SELECT * FROM Product INNER JOIN ProductCategory ON Product.ProdID=ProductCategory.ProdID where CategoryID='$row2[CategoryID]'";
						...
						while($row3 = mysql_fetch_array($rs3))
							{ 
							$javascript_menu_item3.='fx("text='.$row3['ProdCode'].' ;url=vn_productdetail.php?id='.$row3['ProdID'].';target=iframe;width=150");';
							}
...
<td><script language="javascript1.2">fixMenu("Main Menu 2")</script></td>
</tr>
</table>
...
<tr>
<td>
<table><tr><td><script language="javascript1.2">fixMenu("Main Menu 3")</script></td></tr></table>
</td></tr>
</table>

		  </td> 
		  <td width="28"><strong><a href=index.php>En</a></strong></td>
		  </tr>
		  </table>
		  </TD>
        </TR>
                ...
                <TD bgColor=#dcdcdc><TABLE id=table6 cellSpacing=0 cellPadding=0 width="100%" border=0>
                  			<TR>
                                ...
                                summary="Design Table" border=0>
                  																<TR vAlign=top>
                  																	<TD class=Normal 
id=_ctl0__ctl1__ctl0_HtmlHolder><iframe src="vn_menudetail.php?id=0" name="iframe" width="100%" marginwidth="0" height="500px" marginheight="0" align="middle" scrolling="auto" frameborder="0" id="iframe"></iframe></TD>
