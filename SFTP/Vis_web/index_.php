...
if ($recordCount>0)
{
	...
	{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
		...
		{
		$javascript_menu_item0.='fx("text='.$row0['EngName'].';show='.$row0['CategoryID'].';url=contact_.php?id='.$row0['CategoryID'].'&menu0='.$row0['EngName'].';fontweight=bold;target=iframe");';
	...
	while($row1 = mysql_fetch_array($rs1))
	{
		...
		{
			$javascript_menu_item1.='fx("text='.$row1['EngName'].' ;show='.$row1['CategoryID'].';url=contact_.php?id='.$row1['CategoryID'].';target=iframe;width=185");';
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				{
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact_.php?id='.$row2['CategoryID'].';target=iframe;width=185");';
				}
				//level3
				//sql
						...
						while($row3 = mysql_fetch_array($rs3))
							{
								if($row3['Queue']=='0' && $row3['Queue']!='1')
									...
									{
									$javascript_menu_item3.='fx("text='.$row3['EngName'].' ;show='.$row3['CategoryID'].';url=contact_.php?id='.$row3['CategoryID'].';target=iframe;width=185");';
				// start level4
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >4)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='T' ORDER BY Position LIMIT 4, 1";
	$rs0 = mysql_query($SQL);
	...
	{ if ($row0['Queue']=='0' && $row0['Queue']!='1')
		...
		{
		$javascript_menu_item0.='fx("text='.$row0['EngName'].' ;show='.$row0['CategoryID'].';url=contact_.php?id='.$row0['CategoryID'].';fontweight=bold;target=iframe");';
		}
	}
...
//echo $javascript_menu1;
 				...
 				if ($recordCount >8)
{ 
	$SQL = "SELECT CategoryID, EngName, ShowInIndex, Photo, Queue, ActiveStatus, DATE_FORMAT(CreateDate, '%M %d, %Y %H:%i') AS CreateDate, DATE_FORMAT(ModifyDate, '%M %d, %Y %H:%i') AS ModifyDate FROM Menu WHERE PCID=0 and ActiveStatus='T' and ShowInIndex='T' ORDER BY Position LIMIT 6, 1";
	$rs0 = mysql_query($SQL);
		...
		else
		{
			...
			while($row2 = mysql_fetch_array($rs2))
			{
				...
				if (strlen($row2['EngName']) > 20) {
					...
					$javascript_menu_item2.='fx("text='.$row2['EngName'].' ;show='.$row2['CategoryID'].';url=contact_.php?id='.$row2['CategoryID'].';target=iframe;width=185;height=20;");';
				}
				//level3
				//sql
...
height=20;
orientation="horizontal";
fx("text=Vine Cellar ;url=categorydetail.php?id=1;fontweight=bold;show=Product;target=iframe");
                ...
                <TD bgColor=#dcdcdc><TABLE id=table6 cellSpacing=0 cellPadding=0 width="100%" border=0>
                  			<TR>
                                ...
                                summary="Design Table" border=0>
                  																<TR vAlign=top>
                  																	<TD class=Normal 
id=_ctl0__ctl1__ctl0_HtmlHolder><iframe src="contact_.php?id=1" name="iframe" width="100%" marginwidth="0" height="500px" marginheight="0" align="middle" scrolling="auto" frameborder="0" id="iframe"></iframe></TD>
