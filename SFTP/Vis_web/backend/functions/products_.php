...
function GetRecNoInProduct($PCID) {
	$SQL = "SELECT COUNT(ProdCatID) AS num FROM ProductCategory WHERE CategoryID = $PCID";
	$result = mysql_query($SQL);
...
function SelectAllProductInPage($PCID, $startRow, $pageSize) {
	$SQL = "SELECT ProductCategory.ProdCatID, ProductCategory.CategoryID AS PCID, ProductCategory.ActiveStatus AS PCAS, Product.ProdID, Product.ProdCode, Product.EngHeader, Product.ShowInIndex, Product.ActiveStatus FROM ProductCategory, Product WHERE ProductCategory.ProdID = Product.ProdID AND ProductCategory.CategoryID = $PCID ORDER BY Product.Queue LIMIT $startRow,$pageSize";
