<?
// version 2.2 november 2004 Unix version
// for win32 replace with backup_timeout.php
// new indexes included (FULLTEXT / changed UNIQUE)
    ...
    while($row = mysql_fetch_array($result)) {
     ...
     while(list($x, $columns) = @each($index)) {
        ...
        if($x == "PRIMARY") {
           ...
           $def .= " UNIQUE " .substr($x,7)." (";
        }elseif (substr($x,0,8) == "FULLTEXT") {
           $def .= " FULLTEXT ".substr($x,9)." (";
        } else {
          $def .= " KEY $x (";
        }
        $def .= implode($columns,", ") . ")";
  ...
  <TABLE WIDTH="80%" border="0" cellspacing="0" bgcolor="#8BA5C5">
    <TR>
      <TD height="215" valign="top"> <h4>MySQL PHP Backup :: Backup</h4>
        ...
        In the "dump" directory, you should find a file titled <font color="#990000"><b>"backup.sql"</b></font> (without
        the quotes). <br>
        ...
        echo '
        <br> File size: '.round((filesize($path."backup.$filetype")/1024),1). ' kB';
        $endtime=time();
