/*
 *  H.I.M JavaScript Vietnamese Input Method Source File 1.11 build 20050524
 *
...
var dockspell=1 //Start H.I.M with spell checking on or off (1 is on, 0 is off)
var agt=navigator.userAgent.toLowerCase(),alphabet="QWERTYUIOPASDFGHJKLZXCVBNM\ ",them,spellerr
...
function vni_viqr(w,k,i) {
	if((method==2) || (method==0)) {
		DAWEO="6789"; SFJRX="12534"; S="1"; F="2"; J="5"; R="3"; X="4"; Z="0"; D="9"; FRX="234"; AEO="6"; moc="7"; trang="8"; them="678"
	} else if(method==3) {
		DAWEO="^+(D"; SFJRX="'`.?~"; S="'"; F="`"; J="."; R="?"; X="~"; Z="-"; D="D"; FRX="`?~"; AEO="^"; moc="+"; trang="("; them="^+("
	}
...
function telex(w,k,i) {
	SFJRX="SFJRX"; DAWEO="DAWEO"; D='D'; S='S'; F='F'; J='J'; R='R'; X='X'; Z='Z'; FRX="FRX"; them="AOEW"; trang="W"; moc="W"
	var uk=k.toUpperCase(),sf,by
	if(SFJRX.indexOf(uk)>=0) {
		var ret=sr(w,k,i)
		if(ret) return ret
...
function ckspell(word,k) {
	word=unV(word); var exc="UOU,IEU".split(','),z,next=true,noE="UOU,UOI,IEU".split(','),noBE="YEU",noB="YE"
	var check=true,noM="UE,UYE,IU,EU,UY".split(','),noMT="AY,AU".split(','),noT="UA",noT="UA"
	var upword=word.toUpperCase(),tmpword=upword,update=false,gi="IO",noAOEW="OE,OO,AO,EO,IA,AI".split(','),noAOE="OA"
	var notViet="AA,AE,EE,OU,YY,YI,IY,EY,EA,EI,II,IO,YO,YA,YU".split(','),uk=k.toUpperCase(),twE,uw2=unV2(upword)
	...
	if(tmpword!=twE) for(z=0; z<noE.length; z++) if(uw2.indexOf(noE[z])>=0) return true
	if((tmpword!=upword)&&(uw2.indexOf(noBE)>=0)) return true
	uw2=unV2(tmpword)
	if((uw2.indexOf('UYE')>0)&&(uk=='E')) check=false
	if((them.indexOf(uk)>=0)&&(check)) {
		for(g=0; g<noAOEW.length; g++) if(uw2.indexOf(noAOEW[g])>=0) return true
		if(uk!=trang) if(uw2.indexOf(noAOE)>=0) return true
		if(uk==trang) if(uw2.indexOf(noT)>=0) return true
		if(uk==moc) for(g=0; g<noM.length; g++) if(uw2.indexOf(noM[g])>=0) return true
		if((uk==moc)||(uk==trang)) for(g=0; g<noMT.length; g++) if(uw2.indexOf(noMT[g])>=0) return true
	}
	return false
}
...
function findC(word,k,sf) {
	...
	var uk=k.toUpperCase(),i=word.length,uni_array=repSign(k),uuTest=unV2(word.toUpperCase()).indexOf("UU")
	if (DAWEO.indexOf(uk)>=0) {
		if((uk==moc)&&(uuTest>=0)) {
			if(uuTest==(word.length-2)) res=2
			else return false
		} else {
	...
	for(g=1; g<=w.length; g++) {
		for(h=0; h<u.length; h++) {
			if(u[h]==w.charCodeAt(w.length-g)) {
				w=w.substr(0,w.length-g)+fcc(skey[h%24])+w.substr(w.length-g+1)
			}
		}
	}
	return w
	...
	for(g=1; g<=w.length; g++) {
		for(h=0; h<skey.length; h++) {
			if(skey[h]==w.charCodeAt(w.length-g)) {
				w=w.substr(0,w.length-g)+skey2[h]+w.substr(w.length-g+1)
			}
		}
	}
	return w
...
spellerr=(dockspell==0)?nospell:ckspell
